public class HomeTask_1Perform {

    /**
     * Task 1
     */
    public static void performTask1(){
        System.out.println("*****Task_1*****");
        Double var1 = -12.1;
        Double var2 = -12.2;
        Double var3 = -12.2;
        HomeTask_1Utils.Task1(var1, var2, var3);
        System.out.println();
    }
    /**
     * Task 2
     */
    public static void performTask2(){
        System.out.println("*****Task_2*****");
        HomeTask_1Utils.Task2( 1, -7, 12);
        System.out.println();
        double[] arg ={1,-8,13};
        HomeTask_1Utils.Task2(arg);
        System.out.println();
    }

    /**
     * Task 3
     */
    public static void performTask3(){
        System.out.println("*****Task_3*****");
        Long num  = new Long(233344590);
        HomeTask_1Utils.Task3(num);
        System.out.println();
    }

    /**
     * Task 4
     */
    public static void performTask4(){
        System.out.println("*****Task_4*****");
        int[] arg = {12,3,5,-4,7,-90};
        HomeTask_1Utils.Task4(arg);
        System.out.println();
    }

    /**
     * Task 5
     */
    public static void performTask5(){
        System.out.println("*****Task_5*****");
        int[] arg = {12,3,5,-4,7,-90};
        HomeTask_1Utils.Task5(arg);
        System.out.println();
    }

    /**
     * Task 6
     */
    public static void performTask6(){
        System.out.println("*****Task_6*****");
        int[] arg = {12,3,5,-4,7,-90};
        HomeTask_1Utils.Task6(arg);
        System.out.println();
    }

    /**
     * Task 7
     */
    public static void performTask7(){
        System.out.println("*****Task_7*****");
        int arg[] = new int[50];
        for(int i = 0; i < arg.length; i++) {
            arg[i] = (int)(Math.random() * 100);
        }
        HomeTask_1Utils.Task7(arg);
        System.out.println();
    }

    /**
     * Task 8
     */
    public static void performTask8(){
        System.out.println("*****Task_8*****");
        HomeTask_1Utils.Task8(4);
        System.out.println();
    }

    /**
     * Task 9
     */
    public static void performTask9(){
        System.out.println("*****Task_9*****");
        HomeTask_1Utils.Task9();
        System.out.println();
    }

    /**
     * Task 10
     */
    public static void performTask10(){
        System.out.println("*****Task_10*****");
        HomeTask_1Utils.Task10();
        System.out.println();
    }

    /**
     * Task 11
     */
    public static void performTask11(){
        System.out.println("*****Task_11*****");
        HomeTask_1Utils.Task11();
        System.out.println();
    }

}
